...
const getAllData = async (req, res) => {
  ...
  const todayFun = (teachers, staff) => {
    ...
    const today = "Sabtu";
    teachers.map((items) => {
      items.schedule = items.schedule.filter((c) => today.includes(c.day));
      return items;
    });
  };
...
const confirmAccount = async (req, res) => {
  ...
  try {
    const getId = await teacherModels.findById(id);
    if (!getId) {
...
const viewAccount = async (req, res) => {
  ...
  try {
    ...
    const teacherConfirmation = await teacherModels.find({
    ...
    const filterByQuery = (auth, staff, teacher, student) => {
      const authData = [];
      for (let i = 0; i < auth.length; i++) {
        authData.push({
          _id: auth[i]._id,
          identityNumber: auth[i].identityNumber,
          role: auth[i].role,
          createdAt: auth[i].createdAt,
          updatedAt: auth[i].updatedAt,
        });
      }

      // copy all data
      const data = [...teacher, ...staff, ...student];
      ...
      authData.forEach((e) => {
        datas.push({
          ...
          role: e.role,
          fullName:
            data.find((element) => element.identityNumber === e.identityNumber)
              ?.fullName || "",
      ...
      const roleFilter = [];
      authData.map((e) => roleFilter.push(e.role));
      const resultFilter = roleFilter.filter((e) => e === sortQuery);

      if (sortQuery == [...new Set(resultFilter)]) {
        return datas.filter((e) => {
          return e.role == sortQuery;
        });
      }
    ...
    const resutlFilter = filterByQuery(auth, staff, teacher, student);

    if (resutlFilter === false) {
      return res.status(422).json({ message: "Something Broke" });
    }
